...
SECOND_STEP_SYSTEM_TEMPLATE = "two_shot_second_prompt_strict_04.txt"

def two_step_strict(query: str, spreadsheet: Spreadsheet) -> List[Tuple[int, List[str]|None]]:
    return asyncio.run(highlight_worksheets(query, spreadsheet))

async def atwo_step_strict(query: str, spreadsheet: Spreadsheet) -> List[Tuple[int, List[str]|None]]:
...
async def identify_cells_to_highlight(sheet: Worksheet, query: str) -> List[str]:
    ...
    #print(f"DEBUG: two_step_strict final query: {query} -------------> {second_prompt}")
