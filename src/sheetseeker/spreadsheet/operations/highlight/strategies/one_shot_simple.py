...
SYSTEM_PROMPT_TEMPLATE = "highlight_cells_in_sheet_05.txt"

def one_shot_simple(query: str, spreadsheet: Spreadsheet) -> List[Tuple[int, List[str]|None]]:
    return asyncio.run(highlight_worksheets(query, spreadsheet))

async def aone_shot_simple(query: str, spreadsheet: Spreadsheet) -> List[Tuple[int, List[str]|None]]:
    return await highlight_worksheets(query, spreadsheet)

def generate_system_prompt(data: List[List[Any]]) -> str:
...
async def identify_cells_to_highlight(sheet: Worksheet, query: str) -> List[str]:
    system_prompt = generate_system_prompt(prep_data_from_worksheet(sheet))
